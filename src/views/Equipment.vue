...
const confirmDelBatch = () => {
  ...
  request.post('/equipment/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column prop="description" label="器材描述"></el-table-column>
      <el-table-column prop="quantity" label="器材数量"></el-table-column>
      <el-table-column label="图片"><template #default="scope"><el-image preview-teleported style="width: 80px; height: 80px" :src="scope.row.img" :preview-src-list="[scope.row.img]"></el-image></template></el-table-column>
        ...
        <el-table-column label="操作" width="180">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('equipment.edit')">编辑</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" title="器材管理信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        ...
        <el-form-item prop="quantity" label="数量">
          <el-input v-model="state.form.quantity" autocomplete="off"></el-input>
        </el-form-item>
